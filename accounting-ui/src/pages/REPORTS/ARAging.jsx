...
export default function ARAging() {
    ...
    (async () => {
      try {
        ...
        if (genlibRes.ok) {
          ...
          setParties(data.filter((p) => p.type === "CUSTOMER" && p.status === "ACTIVE"));
        }
      } catch (err) {
  ...
  async function generateReport() {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/api/reports/ar-aging?${params.toString()}`, {
        credentials: "include",
        headers: authHeaders(),
      });
      ...
      console.error(err);
      alert("Failed to generate AR Aging Report.");
      setRows([]);
  ...
  function exportCsv() {
    const headers = [
      "customer", "reference_no", "document_date", "due_date", "days_outstanding", "aging_bucket",
    ...
    link.download = `ar-aging-detailed-${asOfDate}.csv`;
    link.click();
  ...
  return (
    <div className="tb-page">
      <div className="tb-header">
        <h1>AR Detailed Aging Report</h1>
      </div>
        ...
        <div className="tb-report-card">
          <div className="tb-report-top">
            ...
            <div className="tb-report-title">
              <h2>ACCOUNTS RECEIVABLE DETAILED AGING REPORT</h2>
            ...
            <table className="tb-report-table aging-table">
              ...
              <tbody>
                ...
                {filteredRows.length === 0 && (
                  <tr>
                    <td colSpan={15} style={{ textAlign: "center", padding: "20px" }}>No outstanding receivables found.</td>
                  </tr>
                )}
              </tbody>
